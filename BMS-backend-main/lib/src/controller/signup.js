...
// Zod to validate new user schema
...
function checkIfUserHasAccount(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const { username, email, password, confirm_password } = validation;
            const existingEmail = yield auth_1.usersModel.findOne({ email });
            if (existingEmail && existingEmail.email === email) {
                ...
                const user = yield auth_1.usersModel.create({
                    username,
                    email,
